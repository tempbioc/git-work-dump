...
while IFS= read -r repo_url; do
    ...
    mkdir -p "logs/$REPO_NAME"
    ( git clone "$repo_url" "$REPO_NAME" || cd "$REPO_NAME"/.. || echo "$repo_url" >> exceptions ) 2>&1 | tee logs/$REPO_NAME/clone_log
    cd "$REPO_NAME" || continue
    git remote remove origin
    git remote add origin "https://github.com/$ORG_NAME/$REPO_NAME.git"
    git push -u origin --all 2>&1 | tee ../logs/$REPO_NAME/push_log 
